...
CHIP_ERROR ConnectivityManagerImpl::_SetWiFiStationMode(WiFiStationMode val)
{
    ...
    if (val != kWiFiStationMode_ApplicationControlled)
    {
        ...
        err              = Internal::TalariaUtils::SetAPMode(autoConnect);
        SuccessOrExit(err);
...
CHIP_ERROR ConnectivityManagerImpl::_GetAndLogWiFiStatsCounters(void)
{
    ...
    if (err != ESP_OK)
    {
        ...
        return TalariaUtils::MapError(err);
    }
    ...
    if (event->Type == DeviceEventType::kESPSystemEvent)
    {
        if (event->Platform.ESPSystemEvent.Base == WIFI_EVENT)
        {
            ...
            case WIFI_EVENT_SCAN_DONE:
                ...
                NetworkCommissioning::TalariaWiFiDriver::GetInstance().OnScanWiFiNetworkDone();
                break;
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
                ...
                NetworkCommissioning::TalariaWiFiDriver::GetInstance().SetLastDisconnectReason(event);
                if (mWiFiStationState == kWiFiStationState_Connecting)
                {
...
void ConnectivityManagerImpl::DriveStationState()
{
    ...
    if (mWiFiStationMode != kWiFiStationMode_ApplicationControlled)
    {
        ...
        ReturnOnFailure(Internal::TalariaUtils::StartWiFiLayer());

        // Ensure that station mode is enabled in the ESP WiFi layer.
        ReturnOnFailure(Internal::TalariaUtils::EnableStationMode());
    }

    // Determine if the ESP WiFi layer thinks the station interface is currently connected.
    ReturnOnFailure(Internal::TalariaUtils::IsStationConnected(stationConnected));
...
void ConnectivityManagerImpl::OnStationDisconnected()
{
    ...
    PlatformMgr().PostEventOrDie(&event);
    WiFiDiagnosticsDelegate * delegate = GetDiagnosticDataProvider().GetWiFiDiagnosticsDelegate();
    uint16_t reason                    = NetworkCommissioning::TalariaWiFiDriver::GetInstance().GetLastDisconnectReason();
...
void ConnectivityManagerImpl::DriveAPState()
{
    ...
    err = Internal::TalariaUtils::IsAPEnabled(espAPModeEnabled);
    SuccessOrExit(err);
    ...
    if (mWiFiAPMode != kWiFiAPMode_ApplicationControlled)
    {
        ...
        err = Internal::TalariaUtils::StartWiFiLayer();
        SuccessOrExit(err);
        ...
        if (mWiFiAPState != targetState)
        {
            ...
            if (targetState == kWiFiAPState_Active)
            {
                ...
                {
                    err = Internal::TalariaUtils::SetAPMode(true);
                    SuccessOrExit(err);
            ...
            else
            {
                ...
                {
                    err = Internal::TalariaUtils::SetAPMode(false);
                    SuccessOrExit(err);
    ...
    if (mWiFiAPState == kWiFiAPState_Active && Internal::TalariaUtils::IsInterfaceUp("WIFI_AP_DEF") &&
        !Internal::TalariaUtils::HasIPv6LinkLocalAddress("WIFI_AP_DEF"))
    {
...
exit:
    if (err != CHIP_NO_ERROR && mWiFiAPMode != kWiFiAPMode_ApplicationControlled)
    {
        ...
        Internal::TalariaUtils::SetAPMode(false);
    }
}
...
CHIP_ERROR ConnectivityManagerImpl::_SetPollingInterval(System::Clock::Milliseconds32 pollingInterval)
{
    ...
    // For Talaria platform, the listen interval of the legacy power-saving mode can only be configured before connecting to AP.
