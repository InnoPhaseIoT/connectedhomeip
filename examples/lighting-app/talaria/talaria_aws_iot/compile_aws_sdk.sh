...
if [ ! -e "${TOP}/apps/talaria_two_aws" ]; then
    ...
    echo "aws_iot_sdk compilation failed, \"<freertos_sdk_x.y>/apps/talaria_two_aws\" not found..."
    echo "please refer to README or \"<freertos_sdk_x.y>/apps/iot_aws/doc/Application_for_IoT_AWS.pdf\" to setup AWS IoT SDK for Talaria TWO Platform"
    exit 1
fi

# Change directory to compile libaws_iot_sdk_t2 and and libaws_iot_sdk_t2_pal
pushd "$TOP/apps/talaria_two_aws/sample_apps/freertos/subscribe_publish_sample" || { echo "aws_iot_sdk compilation failed, invalid directory path"; exit 1; }
